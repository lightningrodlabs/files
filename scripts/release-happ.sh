...
#   1. `artifacts/files.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/files.happ"
else
  happ="artifacts/files.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical files.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download files.happ from $HAPP_TAG."
