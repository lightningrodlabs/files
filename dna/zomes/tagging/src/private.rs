...
pub fn query_all_PrivateTag(_: ()) -> ExternResult<Vec<(EntryHash, Timestamp, String)>> {
   ...
   let res = records.iter().map(|record| {
         ...
         (
            ...
            PrivateTag::try_from(entry).unwrap().value,
         )
      })
      .collect();
   /// Attest entries
...
fn tag_private_entry(input: TaggingInput) -> ExternResult<()> {
   ...
   for tag in tags {
      ...
      let _ = create_link_relaxed(
         tag_eh.clone(),
         input.target.clone(),
         TaggingLinkTypes::PrivateEntry,
         str2tag(&input.link_tag_to_entry.clone()),
      )?;
      let _ = create_link_relaxed(
         input.target.clone(),
         tag_eh,
         TaggingLinkTypes::PrivateTags,
         LinkTag::from(()),
      )?;
   }
   /// Done
...
fn untag_private_entry(input: UntagInput) -> ExternResult<()> {
   ...
   /// Get Tag link
   let link_tuples =
      get_typed_from_links::<PrivateTag>(link_input(input.target.clone(), TaggingLinkTypes::PrivateTags.try_into_filter().unwrap(), None), GetStrategy::Local)?;
...
pub fn find_private_entries_with_tag(tag: String) -> ExternResult<Vec<(EntryHash, String)>> {
   ...
   for tuple in private_tags {
      if tuple.2 == tag {
         /// Found: grab links
         let links = get_links(link_input(tuple.0, TaggingLinkTypes::PrivateEntry.try_into_filter().unwrap(), None), GetStrategy::Local)?; // FIXME strategy
         let res = links
            .clone()
            .into_iter()
