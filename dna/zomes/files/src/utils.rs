...
use zome_files_integrity::{FILE_TYPE_NAME, FILES_DEFAULT_INTEGRITY_ZOME_NAME};
...
pub fn ensure_parcel_is_file(description: &ParcelDescription) -> ExternResult<()> {
   if description.zome_origin != FILES_DEFAULT_INTEGRITY_ZOME_NAME.into() {
      return error(&format!(
         "Parcel is not from '{}' zome. Zome origin: {}",
         FILES_DEFAULT_INTEGRITY_ZOME_NAME, description.zome_origin
      ));
   }
